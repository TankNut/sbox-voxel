...
namespace Voxel
{
	public class MagicaLoader : IVoxelLoader
	{
...
#pragma warning restore CS0649
		...
		public VoxelModel Load( Stream stream )
		{
			...
			List<MagicaVoxel> voxels = new();
			uint[] colors = _defaultColors;

			VoxelBuilder builder = new();

			while ( stream.Position + 1 < stream.Length )
			{
				MagicaChunk chunk = stream.ReadStructureFromStream<MagicaChunk>();
				...
				switch ( chunk.Id )
				{
					case XYZI:
						...
						{
							voxels.Add( stream.ReadStructureFromStream<MagicaVoxel>() );
						}
